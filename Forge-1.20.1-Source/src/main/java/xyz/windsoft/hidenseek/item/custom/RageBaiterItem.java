...
/*
 * This class creates the custom behavior for the item "Rage Baiter"
 *
...
public class RageBaiterItem extends Item {

    //Public methods

    public RageBaiterItem(Item.Properties pProperties) {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        ...
        int ITEM_ADRENALINE_GAIN = Config.hidderRageBaiterAdrenalineIncrease;
        int ITEM_COOLDOWN_SECONDS = Config.hidderRageBaiterCooldown;

        //Add a base description
        tooltip.add(Component.translatable("tooltip.hidenseek.rage_baiter.desc").withStyle(ChatFormatting.BLUE));
        ...
        if (Screen.hasShiftDown() == true){
            tooltip.add(Component.translatable("tooltip.hidenseek.general.rage_baiter_noise_dist").withStyle(ChatFormatting.GRAY)
                    .append(Component.literal(": ").withStyle(ChatFormatting.GRAY))
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        ...
        if (pLevel.isClientSide() == false){
            ...
            if (serverPlayer != null){
                //Repass the call of the Rage Baiter play, for the Game Manager, if is a ServerPlayer
                GameManagerLogic.PlayRageBaiterOfHidder(serverPlayer);

                //Set the new cooldown for the item
                pPlayer.getCooldowns().addCooldown(this, (Config.hidderRageBaiterCooldown * 20));
            }
        }
